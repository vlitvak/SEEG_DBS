...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 22 19:17:23 2025
...
boldmap=mat73.loadmat('boldmap_paper_lv_red_nn/boldmap_list_sc_bin_vect_red.mat')
csv_list=[
...
for i in pt_list:
    ...
    pt=csv_list[i][0:3]
    sc=mat73.loadmat('new_bin_paper/sc_vect_'+pt+'.mat')
    seeds=np.arange(0,np.size(sc['strucgm_vect_red'],0))
    coeff_sc_pt=np.zeros([len(seeds),len(task_list)])
    for j in seeds:
        for k in task_list:
            corr, p=kendalltau(sc["strucgm_vect_red"][j,:],boldmap["map_vect"][k,:])
            coeff_sc_pt[j,k]=corr
    
    coeff_sc_pt={'coeff_sc_pt':coeff_sc_pt}
    scipy.io.savemat('send_fin/coeff_sccorr_'+pt+'.mat',coeff_sc_pt)
