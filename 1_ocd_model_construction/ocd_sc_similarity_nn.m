%%
%construction of structural connectivity from OCD electrodes,
%acquisition of SC overlap and correlations between SC overlap (SC
%correlation) and improvement. SC correlation is acquired in python
%platform due to limitation of memory size. 
seed_file_list=[
    "sub-Cologne01.nii";
    "sub-Cologne02.nii";
    "sub-Cologne03.nii";
    ...
    "sub-GrenobleSAC.nii";
    "sub-GrenobleSEY.nii";
    "sub-LondonALICallp.nii";
    "sub-LondonALICbris.nii";
    "sub-LondonALICdonh.nii";
    "sub-LondonALICmana.nii";
    "sub-LondonALICmarp.nii";
    "sub-LondonALICmcmp.nii";
    "sub-LondonSTNallp.nii";
    "sub-LondonSTNbris.nii";
    "sub-LondonSTNdonh.nii";
    "sub-LondonSTNmana.nii";
    "sub-LondonSTNmarp.nii";
    "sub-LondonSTNmcmp.nii";
    "sub-MadridPatient0100.nii";
    "sub-MadridPatient0101.nii";
    "sub-MadridPatient0102.nii";
    "sub-MadridPatient0103.nii";
    "sub-MadridPatient0200.nii";
    "sub-MadridPatient0201.nii";
    "sub-MadridPatient0202.nii";
    "sub-MadridPatient0203.nii";
    "sub-MadridPatient0300.nii";
    "sub-MadridPatient0301.nii";
    "sub-MadridPatient0302.nii";
    "sub-MadridPatient0303.nii";
    "sub-MadridPatient0400.nii";
    "sub-MadridPatient0401.nii";
    "sub-MadridPatient0402.nii";
    "sub-MadridPatient0403.nii";
    "sub-MadridPatient0500.nii";
    "sub-MadridPatient0501.nii";
    "sub-MadridPatient0502.nii";
    "sub-MadridPatient0503.nii";
    "sub-MadridPatient0700.nii";
    "sub-MadridPatient0701.nii";
    "sub-MadridPatient0702.nii";
    "sub-MadridPatient0703.nii";
    "sub-MadridPatient0900.nii";
    "sub-MadridPatient0901.nii";
    "sub-MadridPatient0902.nii";
    "sub-MadridPatient0903.nii";
    "sub-MadridPatient1000.nii";
    "sub-MadridPatient1001.nii";
    "sub-MadridPatient1002.nii";
    "sub-MadridPatient1003.nii"];
%%
for i=1:length(seed_file_list)
str_connectomes_file='HCP_MGH_32fold_groupconnectome (Horn 2017)';
ea_spherical_roi(seed_file,chan_pos, 3.5 , 0,'leaddbs-develop/templates/space/MNI152NLin2009bAsym/t1.nii');
lead_job(char(seed_file_list(i)),str_connectomes_file);
end
%%
load('boldmap_paper_lv_red_nn/boldmap_list_sc_bin_vect_red');
gm=load_nii('TPM_gm222_corrorg.nii'); %2*2*2mm gray matter template of MNI152NLin2009bAsym
gm.img(isnan(gm.img))=0;
gm.img(isinf(gm.img))=0;
gm_vect=reshape(gm.img,1,[]);
gm_ind=find(gm_vect>0);
for i=1:size(seed_file_list,2)
    roiname=char(seed_file_list(i));
    if exist(['send_fin/',roiname(1:end-4),'_conn-BGPathway_strucmap.nii'])
    strucmap=load_nii(['send_fin/',roiname(1:end-4),'_conn-BGPathway_strucmap.nii']);
    else
        strucmap.img=zeros(91,109,91);
    end
    
    strucmap.img(isnan(strucmap.img))=0;
    strucmap.img(isinf(strucmap.img))=0;
    strucgm_vect(i,:)=reshape(strucgm,1,[]);
    strucgm_vect_red(i,:)=strucgm_vect(i,gm_ind);
for j=1:size(boldmap_list,1)   
    coeff_sc_dot_ocd_uniform(i,j)=sum(sum(sum((map_vect(j,:).*strucgm_vect_red(i,:)))))./sum(map_vect(j,:));
end
end

save('strucgm_vect.mat','strucgm_vect');
save('coeff_sc_dot_ocd_uniform.mat','coeff_sc_dot_ocd_uniform')

%%
coeff_sc_dot_ocd_uniform(isnan(coeff_sc_dot_ocd_uniform))=0;
for i=1:11
[r(i),p(i)]=corr(coeff_sc_dot_ocd_uniform(:,i),improvement,'Type','Spearman');
end
%%
load('coeff_sc_kendall.mat');
coeff_sc_kendall(isnan(coeff_sc_kendall))=0;
for i=1:11
[r(i),p(i)]=corr(coeff_sc_kendall(:,i),improvement,'Type','Spearman')
end
%%
function lead_job(seed_file,str_connectomes_file)
% - Lead-DBS Job created on 02-Nov-2023 16:03:22 -
% --------------------------------------

lead path;

options = getoptslocal(seed_file,str_connectomes_file);

ea_run('run', options);
end

%%
function options = getoptslocal(seed_file,str_connectomes_file);
