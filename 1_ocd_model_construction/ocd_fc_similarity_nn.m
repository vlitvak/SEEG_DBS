%%
%construction of structural connectivity from OCD electrodes and correlations between
%FC correlation and improvement. SC correlation is acquired in python
%platform due to limitation of memory size. Additional FC gray matter
%template is required because FC from Lead DBS has different origin from
%SC.
seed_file_list=[
    "sub-Cologne01.nii";
    "sub-Cologne02.nii";
...
fnc_connectomes_file='GSP 1000 Matrix (Yeo 2011)';
ea_spherical_roi(seed_file,chan_pos, 3.5 , 0,'leaddbs-develop/templates/space/MNI152NLin2009bAsym/t1.nii');
lead_job(char(seed_file_list(i)),fun_connectomes_file);
end
%%
load('boldmap_paper_lv_red_nn/boldmap_list_fc_bin_vect_red');
gm=load_nii('TPM_gm222_corrorg_fnc.nii'); %2*2*2mm gray matter template of MNI152NLin2009bAsym
...
for i=1:length(seed_file_list)
    ...
    fc=load_nii(['/Volumes/WD_BLACK/OCD/vta/',roiname,'_conn-GSP1000MATxFullSet_desc-AvgR_funcmap.nii']);
    fc.img(isnan(fc.img))=0;
    fc.img(isinf(fc.img))=0;
    fc_vect(i,:)=reshape(fc.img,1,[]);
    fc_vect_red(i,:)=fc_vect(i,gm_ind);
end
save('boldmap_paper_lv_red_nn/boldmap_list_fc_bin_vect_red.mat','fc_vect_red');

%%
load('coeff_fc_kendall.mat');
coeff_fc_kendall(isnan(coeff_fc_kendall))=0;
for i=1:11
[r(i),p(i)]=corr(coeff_fc_kendall(:,i),improvement,'Type','Spearman');
end

%%
function lead_job(seed_file,fnc_connectomes_file)
% - Lead-DBS Job created on 02-Nov-2023 16:03:22 -
% --------------------------------------

lead path;

options = getoptslocal(seed_file,fnc_connectomes_file);

ea_run('run', options);
end
%%
function options = getoptslocal(seed_file,fnc_connectomes_file);
...
options.lcm.func.connectome = fnc_connectomes_file;
